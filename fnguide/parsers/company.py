...
class FnGuideCompanyParser:
    """
    FnGuide 기업개요 파싱 클래스
    
    기업개요, https://comp.fnguide.com/SVO2/ASP/SVD_Corp.asp?gicode=A{stock}&ReportGB=D
    """

    # 테이블 유형 판별용 caption 패턴
    KEY_VALUE_CAPTIONS = {"General Information", "일반정보"}
    HISTORY_CAPTIONS = {"최근연혁", "연혁"}
    ...
    def parse(self, stock: str, report_type: str = "D"):
        """
        기업 정보 | 기업개요 정보 추출 후 주요 키를 영어로 변환
        ...
        return self._parse_by_caption(response.text)

    def _parse_by_caption(
        self,
        html: str | Tag
    ) -> Dict[str, Union[TableData, KeyValueData, HistoryData]]:
        """
    ...
    def _parse_all(
        self,
        html: str | Tag
    ) -> List[Union[TableData, KeyValueData, HistoryData]]:
        """
        HTML에서 모든 테이블 파싱
    ...
    def _parse_table(
        self,
        table: Tag
    ) -> Optional[Union[TableData, KeyValueData, HistoryData]]:
        """단일 테이블 파싱 (유형 자동 판별)"""
        ...
        if caption in self.HISTORY_CAPTIONS or self._is_history_table(table):
            return self._parse_history_table(table, caption)
    ...
    def _parse_history_table(self, table: Tag, caption: str) -> Optional[HistoryData]:
        """연혁 테이블 파싱"""
        extractor = HistoryExtractor()
        result = extractor.extract(table)
