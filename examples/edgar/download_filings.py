...
def main():
    ...
    # Download zip file, extract zip file, parse idx file and save tsv file (zip 파일 다운로드, 압축 해세 후 파싱, tsv 파일 저장)
    download_indices(
    ...
    # "cik_tickers": ["AAPL", "GOOGL", "MSFT", "AMZN", "TSLA"]
    ...
    # Parse filings_metadata_filepath file
    old_df = []
    ...
    while True:
        ...
        for year in range(start_year, end_year + 1):
            for quarter in quarters:
                ...
                print(f"Downloading {index_filename} from {url}")
                print(f"{url}에서 {index_filename} 다운로드")

                # Retry the download in case of failures
                # User-agent : SeongJung Kim (sjkim@sayouzone.com)
                ...
                with tempfile.TemporaryFile(mode="w+b") as tmp:
                    ...
                    print(f"\nSaving {os.path.join(indices_folder, index_filename)} file")
                    print(f"{os.path.join(indices_folder, index_filename)} 파일 저장")

                    # Save the processed index file
                    print(f"Writing {os.path.join(indices_folder, index_filename)}")
                    with open(
                        os.path.join(indices_folder, index_filename),
                        "w+",
                        encoding="utf-8",
...
def get_specific_indices(
    ...
    if isinstance(cik_tickers, List) and len(cik_tickers):
        ...
        print(f"Downloading file from {company_tickers_url}")
        print(f"({company_tickers_url})에서 company_tickers.json 파일 다운로드")
        ...
        print(f"Parsing company_tickers.json file")
        print(f"company_tickers.json 파일 파싱")
        ...
        print("Company & ticker:", ticker2cik)
...
def crawl(
    ...
    print(f"\nDownloading html file from {html_index}")
    print(f"{html_index}에서 html file 다운로드")
    ...
    if cik not in company_info_dict:
        ...
        print(f"\nDownloading html file from {company_url}")
        print(f"{company_url}에서 html file 다운로드")
...
def download(url: str, filename: str, download_folder: str, user_agent: str) -> bool:
    ...
    print(f"\nDownloading html file from {url} to {os.path.join(download_folder, filename)} folder")
    print(f"{url}에서 html file 다운로드, {os.path.join(download_folder, filename)} 폴더에 저장")
